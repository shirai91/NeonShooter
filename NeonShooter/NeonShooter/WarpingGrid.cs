...
namespace NeonShooter
{
    public class WarpingGrid
    {
        ...
        PointMass[,] points;
        Vector2 screenSize;
        public WarpingGrid(Rectangle size, Vector2 spacing)
        {
        ...
        public void ApplyDirectedForce(Vector3 force, Vector3 position, float radius)
        {
            foreach (var mass in points)
                if (Vector3.DistanceSquared(position, mass.Position) < radius * radius)
                    mass.ApplyForce(10f * force / (10 + Vector3.Distance(position, mass.Position)));
        }
        public void ApplyImplosiveForce(float force, Vector3 position, float radius)
        {
            foreach (var mass in points)
            {
                ...
                {
                    mass.ApplyForce(10f * force * (position - mass.Position) / (100 + dist2));
        ...
        public void ApplyExplosiveForce(float force, Vector3 position, float radius)
        {
            foreach (var mass in points)
            {
                ...
                if (dist2 < radius * radius)
                {
                    mass.ApplyForce(10f * force * (mass.Position - position) / (10000 + dist2));
                    mass.IncreaseDamping(0.6f);
                }
            }
        }
